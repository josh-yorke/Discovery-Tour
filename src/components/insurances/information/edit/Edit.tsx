...
import { addFaq, getInsuranceFaq } from "../../../../hooks/visa/faqs/faqs";
    ...
    const fetchExistingData = async () => {
      ...
      try {
        ...
        const [
          ...
          getInsurancePricelist(id).catch(() => null),
          getInsuranceProcess(id).catch(() => null),
          getInsurancePayment(id).catch(() => null),
          getInsuranceTerm(id).catch(() => null),
          getInsuranceDocument(id).catch(() => null),
          getInsuranceFaq(id).catch(() => null),
        ]);
...
const Edit = () => {
  ...
  const processFormSubmission = async (
    ...
    for (let i = 0; i < safeData.length; i++) {
      ...
      formDataToSubmit.append("insurance", id!);
