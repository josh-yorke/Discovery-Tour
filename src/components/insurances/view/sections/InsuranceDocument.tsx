...
const InsuranceDocument = ({ insuranceId }: InsuranceDocumentsProps) => {
  ...
  const {
    ...
    queryFn: () => getInsuranceDocument(insuranceId),
    enabled: !!insuranceId,
  });
  ...
  const fileQueries = useQueries({
    queries: allFileIds.map((fileId) => ({
      queryKey: ["insurance-document-file", fileId],
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-xs font-normal text-gray-600">
                All necessary documents for insurance application
              </p>
            </div>
          </div>
          <RiArrowUpSLine
            ...
            <div className="w-full space-y-4 sm:space-y-6">
              {documentsList.map((document) => {
                ...
                const documentFileQueries = fileQueries.filter((q) =>
                  (document.filesAssociated || []).includes(
                    q.data?.file?._id || "",
                  ),
                );
