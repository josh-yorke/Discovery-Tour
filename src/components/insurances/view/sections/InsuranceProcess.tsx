...
interface InsuranceProcessesProps {
  insuranceId: string;
}
...
const InsuranceProcesses = ({ insuranceId }: InsuranceProcessesProps) => {
  ...
  const {
    ...
    queryKey: ["insurance-processes", insuranceId],
    queryFn: () => getInsuranceProcess(insuranceId),
    enabled: !!insuranceId,
  });

  const processesList = processes || [];
  const allFileIds = processesList.flatMap(
    (process) => process.filesAssociated,
  );

  const fileQueries = useQueries({
    queries: allFileIds.map((fileId) => ({
      queryKey: ["insurance-process-file", fileId],
  ...
  const isLoading = isLoadingProcesses || isLoadingFiles;
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-xs font-normal text-gray-600">
                Step-by-step guide for insurance application and processing
              </p>
            </div>
          </div>
          <RiArrowUpSLine
...
export default InsuranceProcesses;
