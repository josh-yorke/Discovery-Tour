...
const InsurancePricelists = ({ insuranceId }: InsurancePriceProps) => {
  ...
  const {
    ...
    queryKey: ["insurance-pricelists", insuranceId],
    queryFn: () => getInsurancePricelists(insuranceId),
    enabled: !!insuranceId,
  });
  ...
  const fileQueries = useQueries({
    queries: allFileIds.map((fileId) => ({
      queryKey: ["insurance-pricelist-file", fileId],
        ...
        <div className="w-full flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-linear-to-r from-[#1d2087] to-[#393ca3] rounded-full">
              <RiPriceTagFill size={20} className="text-white" />
            </div>
            <div className="flex flex-col">
              ...
              <p className="text-xs font-normal text-gray-600">
                Available coverage plans and premiums for this policy
              </p>
            </div>
          </div>
          <RiArrowUpSLine
                  ...
                  <div key={pricelist._id} className="space-y-3 sm:space-y-4">
                    <div className="flex items-start gap-3 sm:gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        {pricelist.coverage &&
                          pricelist.coverage.length > 0 && (
                            <div className="mt-3">
                              <p className="text-xs font-semibold text-gray-800 mb-1">
                                Coverage Includes:
                              </p>
                              <ul className="space-y-1">
                                {pricelist.coverage.map((item, idx) => (
                                  <li
                                    key={idx}
...
export default InsurancePricelists;
