...
const TourPricelists = ({ tourId }: TourPricelistsProps) => {
  ...
  } = useQuery<PricelistData[]>({
    queryKey: ["tour-pricelists", tourId],
    queryFn: () => getTourPricelist(tourId),
    enabled: !!tourId,
  });
  ...
  const fileQueries = useQueries({
    queries: allFileIds.map((fileId) => ({
      queryKey: ["tour-pricelist-file", fileId],
            ...
            {(pricelists || []).length === 0 ? (
              <div className="w-full text-center py-6 sm:py-8">
                <p className="text-gray-500 text-sm sm:text-base">
                  No pricing information available for this tour.
                </p>
              </div>
            ) : (
