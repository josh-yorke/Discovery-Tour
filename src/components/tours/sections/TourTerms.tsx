...
const TourTerms = ({ tourId }: TourTermsProps) => {
  ...
  const {
    ...
    queryKey: ["tour-terms", tourId],
    queryFn: () => getTourTerm(tourId),
    enabled: !!tourId,
  });
  ...
  const isLoading =
    isLoadingTerms || ((terms || []).length > 0 && isLoadingFiles);
            ...
            {(terms || []).length === 0 ? (
              <div className="w-full text-center py-6 sm:py-8">
                <p className="text-gray-500 text-sm sm:text-base">
                  No terms and conditions available for this tour.
                </p>
              </div>
            ) : (
