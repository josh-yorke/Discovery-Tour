...
// The API returns the data directly as an array, not wrapped in a "terms" key
type TermsResponse = TermData[];

interface TransportTermsProps {
  transportId: string;
}
...
const TransportTerm = ({ transportId }: TransportTermsProps) => {
  ...
  const {
    ...
    queryKey: ["transport-terms", transportId],
    queryFn: () => getTransportTerm(transportId),
  ...
  // CRITICAL: Only run useQueries if we have terms with files
  // This prevents the hook from running prematurely
  const allFileIds =
    terms.length > 0 ? terms.flatMap((term) => term.filesAssociated) : [];

  const fileQueries = useQueries({
    queries: allFileIds.map((fileId) => ({
      queryKey: ["transport-term-file", fileId],
      queryFn: () => getVisaFile(fileId),
      enabled: !!fileId && terms.length > 0, // Only enable if fileId exists AND we have terms
      staleTime: 5 * 60 * 1000,
    })),
  });
  ...
  // Important: Only show loading if terms query is loading
  // OR if we have terms and files are loading
  ...
  // CRITICAL: Return null if no terms after loading
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-xs font-normal text-gray-600">
                Important terms and conditions for transportation service
              </p>
            </div>
          </div>
          <RiArrowUpSLine
