...
// The API returns the data directly as an array, not wrapped in a "payments" key
type PaymentsResponse = PaymentData[];
...
const TransportPayment = ({ transportId }: TransportPaymentProps) => {
  ...
  const {
    ...
    queryKey: ["transport-payments", transportId],
    queryFn: () => getTransportPayment(transportId),
  ...
  if (isError) {
    return <SectionError error={error?.message} action={refetch} />;
  }

  // CRITICAL: Return null if no payments after loading
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-xs font-normal text-gray-600">
                Bank account details for transportation service payment
              </p>
            </div>
          </div>
          <RiArrowUpSLine
