...
const Terms = ({ visaId }: TermsProps) => {
  const {
    ...
    queryKey: ["visa-terms", visaId],
    queryFn: () => getVisaTerms(visaId),
    enabled: !!visaId,
  });
      ...
      {terms.length === 0 ? (
        <div className="w-full text-center py-6 sm:py-8">
          <p className="text-gray-500 text-sm sm:text-base">
            No terms and conditions available for this visa.
          </p>
        </div>
      ) : (
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-1 sm:gap-2">
                      ...
                      {term.filesAssociated.length > 0 && (
                        <span className="text-xs text-gray-400 whitespace-nowrap">
                          ...
                          {term.filesAssociated.length !== 1 ? "s" : ""}{" "}
                          attached
                        </span>
                      )}
                    </div>
