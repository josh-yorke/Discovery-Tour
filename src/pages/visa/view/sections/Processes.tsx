...
const Processes = ({ visaId }: ProcessesProps) => {
  const {
    data: processesData,
    ...
    queryKey: ["visa-processes", visaId],
      ...
      {processes.length === 0 ? (
        <div className="w-full text-center py-6 sm:py-8">
          <p className="text-gray-500 text-sm sm:text-base">
            No process information available for this visa.
          </p>
        </div>
      ) : (
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-1 sm:gap-2">
                      ...
                      {process.filesAssociated.length > 0 && (
                        <span className="text-xs text-gray-400 whitespace-nowrap">
                          ...
                          {process.filesAssociated.length !== 1 ? "s" : ""}{" "}
                          attached
                        </span>
                      )}
                    </div>
